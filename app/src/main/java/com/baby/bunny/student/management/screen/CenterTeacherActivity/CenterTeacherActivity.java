        ...
        StringRequest stringRequest=new StringRequest(Request.Method.POST, ApplicationConstant.CenterManager_getAllClass, new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                ...
                try {
                   ...
                   // categories2.add(0, "Select Class");
                 /*   ArrayAdapter adapter = new ArrayAdapter<>( CenterTeacherActivity.this, android.R.layout.simple_spinner_item, selectClassModels);
                    adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
                    centerTeacherViewBind.select_class_spinnerid.setAdapter(adapter);

                    centerTeacherViewBind.select_class_spinnerid.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                        @Override
                        public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                            String sState = categories2.get(position);
                            for (int i = 0; i < selectClassModels.size(); i++) {
                                if (sState.equals(selectClassModels.get(i).getClass_name())) {
                                    //    sStateId = selectClassModels.get(i).getState_id();
                                }
                            }
                        }
